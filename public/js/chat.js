...
var youtubeHistory = [];
var youtubeQueue = [];
var youtubeNowPlaying;
var youtubeApiKey = 'AIzaSyDA8g9sBCkqJXKGvHbx77vj2kdxsX2ZsNQ';
var usersTyping = [];
var nmr = 0;
var dev = true;
var unread = 0;
    ...
    socket.onmessage = function(e) {
        ...
        if (data.type == 'youtube')
        {
            if (data.message == 'status') {
                ...
                youtubeHistory = data.history;
                youtubeNowPlaying = data.nowPlaying;

                $("#youtubeQueue").text("YouTube Queue #" + youtubeQueue.length);
                $("#youtubeHistory").text("YouTube History #" + youtubeHistory.length);
            }
            ...
            if (data.message == 'queue') {
                showChat('youtube', 'YouTube', data.queueMessage);
            }

            if (data.message == 'skip') {
                showChat('youtubeSkip', 'YouTube', data.queueMessage);
            }
    ...
    if(value.length > 0) {
        if(username === undefined) {
            ...
            switch(command[0].toLowerCase()) {
                case 'pm': case 'msg': case 'role': case 'kick': case 'ban': case 'name': case 'alert': case 'me': case 'em': case 'play':
                    if(value.substring(command[0].length).length > 1) {
                        ...
                        if(command[0] == 'play') {
                            var youtubeUrl = value.substring(command[0].length + 2);
                            ...
                            if (match && match[2].length == 11) {
                              ...
                              var youtubeApiUrl = 'https://www.googleapis.com/youtube/v3/videos?id='+videoId+'&key='+youtubeApiKey+'&fields=items(id,snippet(title))&part=snippet';
                              $.getJSON(youtubeApiUrl)
                              .done(function(data) {
                                  if (data.items.length > 0) {
                                      var title = data.items[0].snippet.title;
                                      sendSocket('play', 'youtube', {videoId:videoId, videoTitle:title});
                                  }
                                  else {
                                      showChat('global', 'Error', 'This youtube video does not exist');

                                  }
                              })
                              .fail(function() {
                                console.log("error retrieving youtube title, see network tab for response details");
                                sendSocket('play', 'youtube', {videoId:videoId, videoTitle:videoId});
                              });
                            }
                        }

                        if(command[0] == 'playnext') {
                              sendSocket('next', 'youtube');
                        }
...
$(document).ready(function() {
    ...
    $('#youtubeQueue').bind('click', function() {
        var content = '';
        for (var i = youtubeQueue.length - 1; i >= 0; --i) {
            if(youtubeQueue[i] != undefined) {
                var queue = youtubeQueue[i];
                content += '<li><b>#<a href="https://www.youtube.com/watch?v=' + queue.videoId + '">' + queue.videoTitle + '</a></b>'+
                           ' by <b>' + queue.user + '</b>'+
                           ' queued <b>' + formatDate(queue.queued) + '</b></li>';
            }
        }
        document.getElementById('youtube-queue').innerHTML = content;

        $('#youtube-queue-dialog').modal('show');
    });

    $('#youtubeHistory').bind('click', function() {
        var content = '';
        var userip = '';
        var admin;

        for (var i = youtubeHistory.length - 1; i >= 0; --i) {
            if(youtubeHistory[i] != undefined) {
                var history = youtubeHistory[i];
                content += '<li><b>#<a href="https://www.youtube.com/watch?v=' + history.videoId + '">' + history.videoTitle + '</a></b>'+
                           ' by <b>' + history.user + '</b>'+
                           ' queued <b>' + formatDate(history.queued) + '</b>'+
                           ' played <b>' + formatDate(history.played) + '</b></li>';
            }
        }
        document.getElementById('youtube-history').innerHTML = content;

        $('#youtube-history-dialog').modal('show');
    });
    ...
    $('#playYouTube').bind('change', function() {
        handlePlayYouTubeToggleState();
    });
    ...
    $('#message').keypress(function(e) {
        var youtubeRegex = /^\s*http\S*(youtu\.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*)\S*\s*$/
        var message = $('#message').val().replace(regex, ' ').trim();

        var match = message.match(youtubeRegex);
        if (match && match[2].length == 11) {
            $('#message').val('/play ' + message);
        }
...
function showYouTubePlayer() {
    ...
    playerDiv.appendChild(ytPlayer);
}

function hideYouTubePlayer() {
    var youTubeScript = document.getElementById('www-widgetapi-script');
...
function onPlayerStateChange(event) {
    if (event.data == YT.PlayerState.ENDED) {
        sendSocket('ended', 'youtube', {videoId: player.getVideoData().video_id});
    }
  }
...
function handlePlayYouTubeToggleState() {
    var playYouTube = document.getElementById('playYouTube').checked;
    if (playYouTube)
    {
        showYouTubePlayer();
    }
