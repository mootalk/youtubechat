        ...
        if(rateLimit[conn.id] < 1 && JSON.parse(message).type != 'delete' && JSON.parse(message).type != 'typing' && JSON.parse(message).type != 'ping') {
            ...
            try {
                ...
                if(data.type == 'youtube') {
                    return updateYoutube(conn.id, data);
                }
...
function getNextYouTubeVideo()
{
    ...
    if (youtubeQueue.length > 0) {
        video = youtubeQueue.pop();
    }
    else if(youtubeHistory.length > 0) {
        var randomIndex = Math.floor(Math.random() * youtubeHistory.length);
        video = youtubeHistory[randomIndex];
    }
    return video;
}
...
/* Functions */
function updateYoutube(id, data) {
    var currentVideo = youtubeNowPlaying;
    var shouldPlay = !youtubeNowPlaying;
    var video;


    if (data.message == 'ended') {
        // Another player already said this song ended.
        if (data.extra.videoId != youtubeNowPlaying.videoId)
        {
            return;
        }

        youtubeNowPlaying = null;
        video = getNextYouTubeVideo();

        if (video) {
            video.played = new Date();
            youtubeHistory.push(video);
            shouldPlay = true;

            log('youtube', clients[id].un + ' player ended ' + currentVideo.videoTitle);
        }        
    }
    else if (data.message == 'next') {
        youtubeNowPlaying = null;
        video = getNextYouTubeVideo();
        ...
        if (video) {
            ...
            utils.sendToAll(clients, {
                type: 'youtube',
                message: 'skip',
                queueMessage: clients[id].un + ' didn\'t like the song: ' + currentVideo.videoTitle
            });
            log('youtube', clients[id].un + ' skipped ' + currentVideo.videoTitle);
        }
    }
    else if (data.message == 'play'){
        video = {
            user: clients[id].un, 
            videoId: data.extra.videoId, 
            videoTitle: data.extra.videoTitle,
            queued: new Date(),
            played: new Date()
        };
        ...
        if (shouldPlay) {
            ...
            youtubeQueue.unshift(video);
        }

        utils.sendToAll(clients, {
            type: 'youtube',
            message: 'queue',
            queueMessage: clients[id].un + ' wants to hear ' + video.videoTitle
        });
        log('youtube', clients[id].un + ' queued ' + video.videoTitle);
    }
